...
export async function POST(request: Request) {
  try {
    ...
    if (source === 'llm' || !wordId) {
      wordId = `${wordData.type.substring(0, 3)}-${Date.now()}`
      ...
      if (wordData.type === 'noun') {
      ...
      } else if (wordData.type === 'adverb') {
        vocabInsert.category = wordData.category
        vocabInsert.comparative = wordData.comparative
        vocabInsert.superlative = wordData.superlative
      } else if (wordData.type === 'pronoun') {
        ...
        vocabInsert.declension = wordData.declension
      } else if (wordData.type === 'preposition') {
      ...
      } else if (wordData.type === 'conjunction') {
      ...
      } else if (wordData.type === 'expression') {
      ...
      } else if (wordData.type === 'collocation') {
